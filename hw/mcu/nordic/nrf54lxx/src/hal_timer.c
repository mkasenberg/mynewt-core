...
#define NRF_GRTC_TIMER_CC_INT    (5)
...
hal_grtc_set_ocmp(struct nrf54l_hal_timer *bsptimer, uint32_t expiry)
{
    ...
    /* Extend 32-bit expiry to 64-bit GRTC epoch. */
    expiry64 = ((uint64_t)expiry << CPUTIME_OFFSET) |
               (cntr & (~((uint64_t)((1ULL << (32 + CPUTIME_OFFSET)) - 1))));
    if (expiry64 <= cntr) {
...
hal_grtc_cputime_get(void)
{
    ...
    return (uint32_t)(cntr >> CPUTIME_OFFSET);
}
