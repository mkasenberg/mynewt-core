...
#define BELLBOARD NRF_RADIOCORE_BELLBOARD
...
hal_ipc_signal(uint8_t channel)
{
    ...
    nrf_bellboard_task_trigger(BELLBOARD, nrf_bellboard_trigger_task_get(channel));
...
hal_ipc_enable_irq(uint8_t channel, bool enable)
{
    ...
    for (i = 0U; i < BELLBOARD_NUM_IRQS; i++) {
        ...
        if (enable) {
            nrf_bellboard_int_enable(BELLBOARD, i, BIT(channel));
        } else {
            nrf_bellboard_int_disable(BELLBOARD, i, BIT(channel));
        }
    }
}
...
ipc0_isr(void)
{
    ...
    int_pend = nrf_bellboard_int_pending_get(BELLBOARD, BELLBOARD_IPC0_IRQ);
...
hal_ipc_init(void)
{
    ...
    for (i = 0U; i < BELLBOARD_NUM_IRQS; ++i) {
        nrf_bellboard_int_disable(BELLBOARD, i, evt_mappings[i]);
    }
